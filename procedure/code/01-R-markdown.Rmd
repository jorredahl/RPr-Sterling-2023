...
area-weighted-reaggregation to re-assign casualties from tornado paths to the neighborhoods they passed through, but incorrectly used the term 'areal interpolation' to describe their method. This means that likely Sterling also just used AWR, an assumption that will need to be confirmed.  
...
First, using areal interpolation through area-weighted reaggregation. 
However, because all derived census statistics will be caluculated as percentages, new area-weighted census variables do not need to be calculated. 
Instead, only new polygons clipped by each of the HOLC polygons from the Mapping Inequality Database are used.
