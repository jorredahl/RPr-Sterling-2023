...
packages <- c("tidyverse", "here", "tidycensus", "stepAIC", "margins", "FSA")
...
acs_cb <- get_acs(
  ...
  variables = my_vars,
  state =  c("AL", "AZ", "AR", "CA", "CO", "CT", "DE", "FL", "GA", "ID", "IL", "IN", "IA", "KS", "KY", "LA", "ME", "MD", "MA", "MI", "MN", "MS", "MO", "MT", "NE", "NV", "NH", "NJ", "NM", "NY", "NC", "ND", "OH", "OK", "OR", "PA", "RI", "SC", "SD", "TN", "TX", "UT", "VT", "VA", "WA", "WV", "WI", "WY"),
  year = 2020,
  survey = "acs5",
  geometry = FALSE
)
...
**unplanned deviation from analysis plan**
Due to massive ACS file sizes, we will first run an intersection to only do later data transformations on the block groups that intersect HOLC polygons to speed up the analysis.
```{intersection}
ACS<- st_intersection(ACS_called, HOLC)
```


First, remap ACS values from block groups to HOLC polygons through 'areal interpolation', using area-weighted reaggregation because we assume that's what the authors used. 
...
ACS_clipped <- st_intersection(ACS, HOLC) |>
